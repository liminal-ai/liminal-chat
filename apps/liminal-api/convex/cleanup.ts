...
export const clearTestData = mutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(`Cleanup complete: ${messagesDeleted} messages and ${conversationsDeleted} conversations deleted`);
    return { 
      messagesDeleted, 
      conversationsDeleted,
      usersPreserved: await ctx.db.query("users").collect().then(users => users.length)
    };
  },
});
