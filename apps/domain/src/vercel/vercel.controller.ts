...
export class VercelController {
  private getModel(provider: string = 'openai', model?: string) {
    switch (provider) {
      ...
      default:
        throw new Error(`Unsupported provider: ${provider}. Only 'openai', 'anthropic', 'google', 'openrouter', and 'vercel' are currently enabled.`);
    }
  }
  @Post('chat')
  ...
  async completion(@Body() body: CompletionRequest, @Res() res: FastifyReply) {
    try {
      console.log(`Testing ${body.provider} with model:`, this.getModel(body.provider, body.model));
      const result = streamText({
        model: this.getModel(body.provider, body.model),
        prompt: body.prompt,
      });
  ...
  async useObject(@Body() body: UseObjectRequest, @Res() res: FastifyReply) {
    const schema = body.schema || z.object({
  ...
  @Post('generate-text')
  async generateText(@Body() body: GenerateTextRequest, @Res() res: FastifyReply) {
    try {
