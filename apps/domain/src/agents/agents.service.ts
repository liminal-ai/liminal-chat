...
import type { AgentType, CreateAgentTypeDto, UpdateAgentTypeDto } from './dto/agent-type.dto';
import type { AgentInstance, CreateAgentInstanceDto, UpdateAgentInstanceDto, ExecuteAgentDto } from './dto/agent-instance.dto';
...
export class AgentsService {
  private readonly dataPath = join(process.cwd(), '..', 'domain-data', 'agents');
  ...
  async createAgentType(dto: CreateAgentTypeDto): Promise<AgentType> {
    ...
    const invalidTools = this.toolRegistry.validateToolAccess(validatedDto.toolAccess);
    ...
    try {
      ...
      throw new ConflictException(`Agent type '${validatedDto.typeName}' already exists`);
    } catch (error) {
  ...
  async getAgentType(typeName: string): Promise<AgentType> {
    ...
    try {
      ...
      throw new BadRequestException(`Failed to read agent type: ${error.message}`);
    }
  }
  ...
  async updateAgentType(typeName: string, dto: UpdateAgentTypeDto): Promise<AgentType> {
  ...
  async deleteAgentType(typeName: string): Promise<void> {
    ...
    const dependentInstances = instances.filter(instance => instance.typeName === typeName);
  ...
  async createAgentInstance(dto: CreateAgentInstanceDto): Promise<AgentInstance> {
    ...
    await fs.writeFile(instancePath, JSON.stringify(validatedInstance, null, 2));
  ...
  async getAgentInstance(agentId: string): Promise<AgentInstance> {
    ...
    try {
      ...
      throw new BadRequestException(`Failed to read agent instance: ${error.message}`);
    }
  }
  ...
  async updateAgentInstance(agentId: string, dto: UpdateAgentInstanceDto): Promise<AgentInstance> {
  ...
  async executeAgent(agentId: string, dto: ExecuteAgentDto): Promise<{ response: string; usage?: any }> {
    ...
    const tools = this.toolRegistry.createVercelTools(agentType.toolAccess, context);
  ...
  private async updateAgentContext(agentId: string, interaction: any): Promise<void> {
    const instance = await this.getAgentInstance(agentId);
